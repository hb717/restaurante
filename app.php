<?php 
	...
	function listarComentario() {
		$cnn = abrirConexao();
		$arr = listar($cnn, "SELECT id, ip, data_publicacao, texto, nome, email, bloqueado  FROM comentario");
		fecharConexao($cnn);
		return $arr;
	}
	

//////////////  ID, IP, DATA_PUBLICACAO, TEXTO, BLOQUEADO

	function incluirComentario($dataPublicacao, $ip, $texto, $bloqueado) {
		$cnn = abrirConexao();
		$dataPublicacao = mysqli_real_escape_string($cnn, $dataPublicacao);
		$ip = mysqli_real_escape_string($cnn, $ip);
		$texto = mysqli_real_escape_string($cnn, $texto);
		$bloquado = mysqli_real_escape_string($cnn, $bloquado);
		//$idSessao = mysqli_real_escape_string($cnn, $idSessao);
		$id = incluir($cnn, "INSERT INTO comentario VALUES (NULL,'$dataPublicacao', '$ip', '$texto', $bloqueado)");
		fecharConexao($cnn);
		return $id;
	}
	
	function alterarComentario($id, $dataPublicacao, $ip, $texto, $bloqueado) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$dataPublicacao = mysqli_real_escape_string($cnn, $dataPublicacao);
		$titulo = mysqli_real_escape_string($cnn, $ip);
		$texto = mysqli_real_escape_string($cnn, $texto);
		$ativo = mysqli_real_escape_string($cnn, $bloqueado);
		//$idSessao = mysqli_real_escape_string($cnn, $idSessao);
		$con = alterarOuExcluir($cnn, "UPDATE comentario SET data_publicacao='$dataPublicacao', ip='$ip', texto='$texto', bloqueado=$bloqueado WHERE id=$id");
		fecharConexao($cnn);
		return $con;
	}
	function excluirComentario($id) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$con = alterarOuExcluir($cnn, "DELETE FROM comentario WHERE id = $id");
		fecharConexao($cnn);
		return $con;
	}
	function consultarComentario($id) {
		$cnn = abrirConexao();
		$id = mysqli_real_escape_string($cnn, $id);
		$arr = consultar($cnn, "SELECT c.*, s.nome AS nome_sessao FROM conteudo c INNER JOIN sessao s ON s.id = c.sessao_id WHERE c.id = $id");
		fecharConexao($cnn);
	...
	function listarComentario2($idSessao, $ativo) {
		$cnn = abrirConexao();
